...
from utils.filters.low_pass import butter_filter_rt, lowpass_filter_rt
...
class AbstractController(ABC):
    ...
    def init_filter(self):
        ...
        """
        if self.filt == "butter":
            self.filter = butter_filter_rt(
                dof=self.dof,
                cutoff=self.filt_kwargs["butter_cutoff"],
                x0=self.filt_x0,
            ...
            A, B = self.filt_plant.linear_matrices(
                self.filt_kwargs["kalman_xlin"], self.filt_kwargs["kalman_ulin"]
            )
    ...
    def filter_measurement(self, x, last_u):
        ...
        if self.filt_velocity_cut > 0.0:
            x_filt[self.dof :] = [
                np.where(np.abs(x_filt[i]) < self.filt_velocity_cut, 0, x_filt[i])
                for i in range(self.dof, 2 * self.dof)
            ]
    ...
    def get_friction_torque(self, x):
        ...
        friction_regressor_mat = yb_friction_matrix(
            [x[i] for i in range(self.dof, 2 * self.dof)]
        )
