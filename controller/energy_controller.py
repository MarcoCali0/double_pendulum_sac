...
class EnergyController(AbstractController):
    ...
    def set_parameters(self, kp=1.0, kd=1.0, kv=1.0, ke=1.0):
        """set_parameters.
    ...
    def check_parameters(self):
        if self.robot == "pendubot":
            self.check_parameters_pendubot()
        elif self.robot == "acrobot":
            self.check_parameters_acrobot()

    def check_parameters_acrobot(self):
        """
        Check if the parameters fulfill the convergence conditions presented in
        the paper.
        """
        a1 = self.inertia[0] + self.mass[1] * self.length[0] ** 2.0
        a2 = self.inertia[1]
        ...
        # kp
        p_val = 2 / np.pi * np.min([b1**2.0, b2**2.0])
        ...
        if self.kd > d_val:
            ...
            print(
                f"Kd={self.kd} does NOT fulfill the convergence property kd > {d_val}"
            )
    ...
    def check_parameters_pendubot(self):
        m1, m2 = self.mass
        l1, l2 = self.length
        r1, r2 = self.com
        g = self.gravity
        ...
        alpha1 = m1 * r1**2 + m2 * l1**2 + J1
        k_Ds = 2 * alpha1 * self.desired_energy

        if self.kd / self.ke > k_Ds:
            print(f"KD/KE = {self.kd/self.ke} satisfies condition > K_Ds = {k_Ds}")
        else:
            print(
                f"KD/KE = {self.kd/self.ke} DOES NOT satisfy condition > K_Ds = {k_Ds}"
            )

        beta1 = (m1 * r1 + m2 * l1) * g
        k_Ps = 2 * beta1**2 / np.sqrt(1 + np.pi**2)

        if self.kp / self.ke > k_Ps:
            print(f"KP/KE = {self.kp/self.ke} satisfies condition > K_Ps = {k_Ps}")
        else:
            print(
                f"KP/KE = {self.kp/self.ke} DOES NOT satisfy condition > K_Ps = {k_Ps}"
            )
    ...
    def get_control_output_acrobot(self, x, t=None):
        pos = np.copy(x[:2])
        vel = np.copy(x[2:])
        ...
        u2 = -(
            (self.kv * vel[1] + self.kp * pos[1]) * Del
            + self.kd
            * (M[1][0] * (H[0] - G[0] + F[0]) - M[0][0] * (H[1] - G[1] + F[1]))
    ...
    def get_control_output_pendubot(self, x, t=None):
        pos = np.copy(x[:2])
        vel = np.copy(x[2:])
        ...
        energy = self.plant.total_energy(x)
        self.en.append(energy)
        deltaE = energy - self.desired_energy
        M_inv = np.linalg.inv(M)
        Lambda = self.ke * deltaE + self.kd * B.T @ M_inv @ B
        Lambda_inv = 1 / Lambda
        u1 = Lambda_inv * (
            self.kd * B.T @ M_inv @ (H - G + F)
            - self.kv * vel[0]
            - self.kp * (np.pi - pos[0])
        )
    ...
    def get_control_output_(self, x, t=None):
        ...
        if self.robot == "acrobot":
            return self.get_control_output_acrobot(x, t)
        elif self.robot == "pendubot":
            return self.get_control_output_pendubot(x, t)
    ...
    def save_(self, save_dir):
        ...
        with open(
            os.path.join(save_dir, "controller_energy_Xin_parameters.yml"), "w"
        ) as f:
...
def kd_func(q2, a1, a2, a3, b1, b2, Er):
    ...
    Er : float
    """
    Del = a1 * a2 - (a3 * np.cos(q2)) ** 2.0
    Phi = np.sqrt(b1**2.0 + b2**2.0 + 2.0 * b1 * b2 * np.cos(q2))
    M11 = a1 + a2 + 2.0 * a3 * np.cos(q2)
